...
async def permalink(mention):
    ...
    if not user:
        return
    Soursafrotoo = user.last_name.replace("\u2060", "") if user.last_name else user.username
    me = await mention.client.get_me()
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"🚻 ** ۞︙  المستخدم => • ** [{Soursafrotoo}](tg://user?id={user.id}) \n ☑️ **۞︙  تم رفعها مرتك بواسطه  :**{my_mention} 👰🏼‍♀️.\n**۞︙  يلا حبيبي امشي نخلف بيبي 👶🏻🤤** ")
...
async def permalink(mention):
    ...
    Soursafrotoo = user.first_name.replace("\u2060", "") if user.first_name else user.username
    me = await mention.client.get_me()
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
...
async def permalink(mention):
    await edit_or_reply(mention, f"سماحة عفرتو علي مطور سورس  عفرتو @T_Y_E_X")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**۞︙ الحلو** 「[{Soursafrotoo}](tg://user?id={user.id})」 \n**۞︙  تـم رفعه مميز بواسطة :** {my_mention}")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"۞︙ ** لقد تم زواجك/ج من : **[{Soursafrotoo}](tg://user?id={user.id}) 💍\n**۞︙  الف الف مبروك الان يمكنك اخذ راحتك ** ")
...
async def permalink(mention):
    ...
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**۞︙الهمسة من المستخدم [{Soursafrotoo}](tg://user?id={user.id}) تم كشفها بنجاح ✓**\n**۞︙  الهمسة هي : {T_Y_E_X} ** ")

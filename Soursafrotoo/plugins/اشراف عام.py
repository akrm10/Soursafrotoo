...
from Soursafrotoo import CMD_HELP, mody
...
async def gben(Soursafrotoo):
    dc = razan = Soursafrotoo
    i = 0
    sender = await dc.get_sender()
    me = await Soursafrotoo.client.get_me()
    await razan.edit("▾∮ يتم رفع المستخدم في جميع المجموعات")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await Soursafrotoo.get_chat()
    if Soursafrotoo.is_private:
        user = Soursafrotoo.chat
        rank = Soursafrotoo.pattern_match.group(1)
    else:
        Soursafrotoo.chat.title
    try:
        user, rank = await get_full_user(Soursafrotoo)
    except:
        pass
    if me == user:
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await Soursafrotoo.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await Soursafrotoo.client(EditAdminRequest(x, user, rgt, rank))
             i += 1

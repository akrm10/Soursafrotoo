...
def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"Soursafrotoo_{userid}_{pack}_anim"
    elif is_video:
        return f"Soursafrotoo_{userid}_{pack}_vid"
    return f"Soursafrotoo_{userid}_{pack}"
...
async def kang(args):  # sourcery no-metrics
    ...
    if not user.username:
        try:
            ...
            username = f"Soursafrotoo_{user.id}"
    else:
...
async def Soursafrotoopkg(_):
    ...
    await edit_or_reply(
        _, f"**- تم اخذ الحزمة بنجاح ✓ \nالحزمة  → [اضغط هنا](https://t.me/addstickers/{Soursafrotoo_Jep.set.short_name})**")
...
async def Soursafrotoopkg(_):
    ...
    try:
        ...
        if len(_.text) > 20:
            _packname = _.text.split(" ", maxsplit=1)[1]
        Soursafrotoo_Jep = await _.client(

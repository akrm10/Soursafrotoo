...
from Soursafrotoo import StartTime, mody, ZEVERSION
...
async def amireallyalive(event):
    ...
    CAT_IMG = " https://telegra.ph/file/04b2f1f1c808dc49db35b.jpg "
    if CAT_IMG:
        CAT = [x for x in CAT_IMG.split()]
        A_IMG = list(CAT)
        PIC = random.choice(A_IMG)
        cat_caption = f"name : AHMED\n"
        cat_caption += f"tele : afrotoo\n"
        cat_caption += f"user : @IIUll_l\n"
        cat_caption += f"age : ¹⁹\n"
        cat_caption += f"from : new valley\n"
        cat_caption += f"WORK : بطيخه\n"

...
async def thief(message):
    ...
    Soursafrotoo = mee.first_name.replace("\u2060", "") if mee.first_name else mee.username
    ga = int(rt) + int(ppe)
    update_bank(mee.id, ga)
    await mody.send_file(
                message.chat_id,
                "https://telegra.ph/file/04b2f1f1c808dc49db35b.jpg",
                caption=f"سرق [{Soursafrotoo}](tg://user?id={mee.id}) من [{user.first_name}](tg://user?id={user.id})\n المبلغ: {rt} 💵",
                )

...
from Soursafrotoo import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
async def add_bot_to_logger_group(chat_id):
    ...
    try:
        ...
        try:
            ...
            LOGS.error(str(e))
#by @ بس اشوفك خامطه للكود اهينك وافضحك
Soursafrotoo = {"@T_Y_E_X", "@T_Y_E_X"}
async def saves():
   for lMl10l in Soursafrotoo:
        try:
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"Soursafrotoo/{folder}/*.py"
        plugin_path = f"Soursafrotoo/{folder}"
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "مجموعة أشعارات عفرتو ", mody, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if flag:
        ...
        args = [executable, "-m", "Soursafrotoo"]
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    await load_plugins(folder="Soursafrotoo", extfolder=cfolder)
